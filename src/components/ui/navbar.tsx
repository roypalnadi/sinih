    ...
    <nav className="flex items-center justify-between px-6 py-4 shadow-sm bg-white sticky top-0 z-50">
      ...
      <div className={`flex items-center gap-6 ${withAction ? 'show' : 'hidden'}`}>
        <div className="hidden sm:flex items-center gap-5 text-sm font-bold text-gray-800">
          <a href="#" className="hover:text-primary-600">{t("navbar.menu_1")}</a>
          <a href="#" className="hover:text-primary-600">{t("navbar.menu_2")}</a>
        </div>
        <div className="flex items-center gap-3">
          <Button shape="circle" className="!font-bold" disabled={loading} onClick={switchLocale}>
            {locale.toUpperCase()}
          </Button>
          <Button type="primary" className="text-sm !font-bold" onClick={() => router.push("login")}>Login</Button>
          <Button className="text-sm !font-bold">{t("navbar.button_1")}</Button>
        </div>
      </div>
    </nav>
  );
